...
class PsbReader:
    ...
    def _unpack(self, pos: int) -> Tuple[Any, int]:
        type_byte = self._byte(pos)
        payload = pos + 1
        ...
        if type_byte == PsbObjType.OBJECTS:
            return self._read_objects(payload)
    ...
    def _read_list(self, pos: int) -> Tuple[List[Any], int]:
        offsets, base = self._read_array(pos)
        values: List[Any] = []
        end = base
        for offset in offsets:
            value, item_end = self._unpack(base + offset)
            values.append(value)
            end = max(end, item_end)
        return values, end

    def _read_objects(self, pos: int) -> Tuple[Dict[str, Any], int]:
