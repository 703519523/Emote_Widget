...
class PsbNormalizer:
    """Unwrap, fully parse, checksum-validate and return canonical raw PSB."""
    ...
    def normalize_with_summary(self) -> NormalizeResult:
        ...
        root = parsed["root"]
        summary = {"source": str(self.path), "shell": unwrapped.shell, "source_size": self.path.stat().st_size, "unwrapped_size": len(unwrapped.data), "pure_size": len(decrypted.data), "version": parsed["version"], "header_encrypt": header["header_encrypt"], "source_header_encrypted": decrypted.header_was_encrypted, "source_body_encrypted": decrypted.body_was_encrypted, "crypt_key": decrypted.key, "crypt_key_source": decrypted.key_source, "checksum_valid": parsed["checksum_valid"], "type": parsed["type"], "spec": spec, "name_count": len(parsed["names"]), "string_count": len(parsed["strings"]), "resource_count": len(parsed["resources"]), "extra_resource_count": len(parsed["extra_resources"]), "resources": parsed["resources"], "extra_resources": parsed["extra_resources"], "root_keys": list(root.keys()) if isinstance(root, dict) else []}
